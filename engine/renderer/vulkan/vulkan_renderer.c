...
static void framebuffer_resize_callback(GLFWwindow *win, int w, int h) {
    ...
    if (ctx != nullptr) ctx->framebuffer_resized = true;
}
...
static bool recreate_swapchain(VulkanContext *ctx) {
    ...
    if (!vulkan_swapchain_create(ctx, (uint32_t)w, (uint32_t)h)) return false;
    if (!vulkan_framebuffers_create(ctx)) return false;
...
static bool vulkan_init(Renderer *self) {
    ...
    if (!vulkan_commands_create(ctx)) return false;
...
static bool vulkan_begin_frame(Renderer *self) {
    ...
    vkWaitForFences(ctx->device, 1, &ctx->in_flight[frame],
                    VK_TRUE, UINT64_MAX);

    // Acquire next swapchain image.
    VkResult res = vkAcquireNextImageKHR(
        ctx->device, ctx->swapchain.handle, UINT64_MAX,
        ctx->image_available[frame], VK_NULL_HANDLE, &ctx->image_index);
...
static void vulkan_end_frame(Renderer *self) {
    ...
    if (vkQueueSubmit(ctx->graphics_queue, 1, &submit,
                      ctx->in_flight[frame]) != VK_SUCCESS) {
